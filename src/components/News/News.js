    ...
    return (
        <>
            {articles.length === 0 ? <Spinner /> :
                <div className="w-[100%] m-0 bg-black" style={{ padding: "4rem" }}>
                    <div className="text-3xl text-center text-white">TRENDY FINANCE HEADLINES</div>
                    {
                        <InfiniteScroll
                            pageStart={0}
                            ...
                            threshold={-10}
                        >
                            <div className="row my-3 flex flex-wrap justify-evenly" key={Math.random() + 1}>
                                {renderNews(0, articleEnd)}
                            </div>
                        </InfiniteScroll>
                    }
